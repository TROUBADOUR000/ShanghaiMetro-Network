let sh_subway = new subwayGraph() //生成类
load_data() // 加载数据
...
svg.append('rect').attr('width', width).attr('x', 0).attr('y', 0)
    .attr('height', height).attr('fill', 'None').attr('stroke', 'blue')
...
// 添加group， transform 属性用于移动坐标轴在 svg 中的位置。
let g = svg.append('g').attr('transform', `translate(${margin.left},${margin.top})`)

// 定义zoom，控制图例缩放1-40倍 “zoom”缩放事件发生
const zoom = d3.zoom().scaleExtent([1, 40]).on("zoom", zoomed);
svg.call(zoom)

function zoomed(event) {
    const {transform} = event; //从event类中取出transform属性
...
function draw_line(key1, key2) {
    ...
    for (color of color_set) {   // 渲染边
        let line_name = set_to_string(sh_subway.get_common_lines(key1, key2))
        ...
        g.append('line').attr('x1', start_x + float_pos).attr('y1', start_y + float_pos)
            ...
            .on("mouseover", function () {
                ...
                new_g.append('text').text(line_name)
                    .attr('x', mid_x).attr('y', mid_y).attr('font-size', 8).attr('text-anchor', 'middle')
            })
...
function draw_text(key) {
    ...
    g.append('text').text(d.id).attr('x', start_x + d.float_x).attr('y', start_y + d.float_y).attr('font-size', 3)
        .attr('opacity', 0.5)
        .on("mouseover", function () {
            d3.select(this)
                .attr('opacity', 1)
...
function draw_map() {
    ...
    for (let d of sh_subway.vertex_map.values()) {
        // 先渲染边
        for (let station of d.map.keys()) {
